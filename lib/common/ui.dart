...
class Ui {
  ...
  static Widget dateNotFound({message = 'Data not found'}) {
    return Center(
      child: Padding(
  ...
  static campaignAds(Campaign camp, index, List<Campaign> campaign,
      VideoPlayerController videoplayer, bool isFirstLogin) {
    if (!(camp.condition == 'login' && isFirstLogin)) {
      if (campaign.length > index + 1)
        Ui.campaignAds(campaign.elementAt(index + 1), index + 1, campaign,
            videoplayer, isFirstLogin);
      return;
    } else if (camp.condition == 'firstserv') return;
    var isImage = true;
    if (camp.image.url.contains('.mp4')) {
      videoplayer = VideoPlayerController.network(camp.image.url);
      isImage = false;
    }
    ...
    Get.bottomSheet(
        Material(
            ...
            child: Stack(
              children: [
                Container(
                    height: (MediaQuery.of(Get.context).size.height / 2) + 70,
                ...
                Container(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          if (camp.redirectUrl != null &&
                              camp.redirectUrl != '')
                            MaterialButton(
                              minWidth: 150.0,
                              padding: EdgeInsets.fromLTRB(40, 12, 40, 12),
                              onPressed: () {
                                _launchURL(camp.redirectUrl);
                                Get.back();
                                if (campaign.length > index + 1)
                                  Ui.campaignAds(
                                      campaign.elementAt(index + 1),
                                      index + 1,
                                      campaign,
                                      videoplayer,
                                      isFirstLogin);
    ...
    if (!isImage) videoplayer.initialize().then((value) => videoplayer.play());
  }
