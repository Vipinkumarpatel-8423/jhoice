...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<LaravelApiClient> init() async {
    if (foundation.kIsWeb || foundation.kDebugMode) {
      ...
      _optionsCache =
          buildCacheOptions(Duration(minutes: 10), forceRefresh: true);
  ...
  Future<List<EService>> getRecommendedEServices() async {
    ...
    var _queryParameters = {
      'only':
          'id;name;price;discount_price;price_unit;has_media;media;total_reviews;rate;eProvider',
      'search': 'categories.type:customer,both;',
      'searchFields': 'categories.type:in;',
      'limit': '6',
    };
    ...
    var response = await _httpClient.getUri(_uri, options: _optionsCache);
    if (response.data['success'] == true) {
      List<EService> eServiceList = response.data['data']
          .map<EService>((obj) => EService.fromJson(obj))
          .toList();

      if (eServiceList.length < 1) {
        eServiceList = await getAllEServicesIfNoAddress();
      }
  ...
  Future<List<EService>> getAllEServicesWithPagination(
    ...
    var _queryParameters = {
      ...
      'search': 'categories.type:customer,both;categories.id:$categoryId;',
      'searchFields': 'categories.type:in;categories.id:=;',
      'limit': '4',
      'searchJoin': 'and',
  ...
  Future<List<EService>> getAllEServicesIfNoAddress() async {
    // TODO get Only Recommended
    var _queryParameters = {
      'only':
  ...
  Future<List<EService>> searchEServices(
    ...
    var _queryParameters = {
      'with': 'eProvider;eProvider.addresses;categories',
      'search':
          'categories.type:customer,both;categories.id:${categories.join(',')};name:${keywords}',
      'searchFields': 'categories.type:in;categories.id:in;name:like',
      'searchJoin': 'and',
    };
    if (!_address.isUnknown()) {
      _queryParameters['myLat'] = _address.latitude.toString();
  ...
  Future<List<Favorite>> getFavoritesEServices() async {
    ...
    var _queryParameters = {
      ...
      'search':
          'categories.type:customer,both;user_id:${authService.user.value.id}',
      'searchFields': 'categories.type:in;user_id:=',
      'searchJoin': 'and',
      'orderBy': 'created_at',
      'sortBy': 'desc',
      'api_token': authService.apiToken,
  ...
  Future<List<EService>> getEProviderFeaturedEServices(
    ...
    var _queryParameters = {
      'with': 'eProvider;eProvider.addresses;categories',
      'search':
          'categories.type:customer,both;e_provider_id:$eProviderId;featured:1',
      'searchFields': 'categories.type:in;e_provider_id:=;featured:=',
      'searchJoin': 'and',
      'limit': '5',
  ...
  Future<List<EService>> getEProviderPopularEServices(
    ...
    var _queryParameters = {
      'with': 'eProvider;eProvider.addresses;categories',
      'search': 'categories.type:customer,both;e_provider_id:$eProviderId',
      'searchFields': 'categories.type:in;e_provider_id:=',
      'searchJoin': 'and',
      'rating': 'true',
      'limit': '4',
      'offset': ((page - 1) * 4).toString()
  ...
  Future<List<EService>> getFeaturedEServices(
    ...
    var _queryParameters = {
      ...
      'with': 'eProvider;eProvider.addresses;categories',
      'search':
          'categories.type:customer,both;categories.id:$categoryId;featured:1',
      'searchFields': 'categories.type:in;categories.id:=;featured:=',
      'searchJoin': 'and',
      'limit': '4',
      'offset': ((page - 1) * 4).toString()
    };
  ...
  void sendNotification2(
    ...
    var request = http.MultipartRequest(
        'POST',
        Uri.parse(
            'https://jhoice.com/api/new_notify?users=${users}&from=${from}&text=${text}'));
    request.fields
        .addAll({'users': "${users}", 'from': from.id, 'text': text, 'id': id});
  ...
  Future<bool> sendTrackBooked(trackid) async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ addReview() ]");
    ...
    print('parameter $_queryParameters');
    Uri _uri =
        getApiBaseUri("trackBooked").replace(queryParameters: _queryParameters);
    Get.log(_uri.toString());
    var response =
        await _httpClient.postUri(_uri, data: null, options: _optionsNetwork);
    print('track res: $response');
    if (response.data['success'] == true) {
      return true;
    } else {
      throw false;
  ...
  Future<int> sendClickTrack(service_id, type) async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ addReview() ]");
    ...
    Uri _uri =
        getApiBaseUri("trackclick").replace(queryParameters: _queryParameters);
    Get.log(_uri.toString());
    var response =
        await _httpClient.postUri(_uri, data: null, options: _optionsNetwork);
    if (response.data['success'] == true) {
  ...
  Future<List<Campaign>> getCampigns() async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ getCampaign() ]");
    ...
    Uri _uri =
        getApiBaseUri("campaign").replace(queryParameters: _queryParameters);
    Get.log(_uri.toString());
    var response = await _httpClient.getUri(_uri, options: _optionsCache);
    if (response.data['success'] == true) {
      return response.data['data']
          .map<Campaign>((obj) => Campaign.fromJson(obj))
          .toList();
    } else {
      throw new Exception(response.data['message']);
    }
