...
class BookingView extends GetView<BookingController> {
  ...
  Widget build(BuildContext context) {
    ...
    if (controller.booking.value.expiringAt.isNotEmpty) {
      String hoursStr = controller.booking.value.expiringAt.split(":")[0];
      String minuteStr = controller.booking.value.expiringAt.split(":")[1];

      int hours = int.tryParse(hoursStr);
      int minutes = int.tryParse(minuteStr);

      timeRemaining = controller.booking.value.createdAt
          .add(Duration(hours: hours, minutes: minutes))
          .difference(DateTime.now())
          .inMilliseconds;

      if (timeRemaining > 0) {
        duration = DateTime.now().millisecondsSinceEpoch + timeRemaining;
      }
    }
              ...
              SliverToBoxAdapter(
                child: Column(
                  ...
                  children: [
                    ...
                    controller.booking.value.mode.isNotEmpty? BookingTilWidget(
                          title: Text("Mode of Service",
                              style: Get.textTheme.subtitle2),
                          content: Text(
                              controller.booking.value.mode == 'both'
                                  ? "This service can be done both online and offline"
                                  : controller.booking.value.mode == 'online'
                                      ? "This service will done online"
                                      : controller.booking.value.mode == 'offline'
                                          ? "This service will be done offline"
                                          : "Not Mentioned",
                              style: Get.textTheme.bodyText1),
                        ):Container(),
                        controller.booking.value.location.isNotEmpty? BookingTilWidget(
                          title: Text("Location of Service",
                              style: Get.textTheme.subtitle2),
                          content: Text(
                              controller.booking.value.location == 'both'
                                  ? "Provider can come to your house and you will also visit provider's office"
                                  : controller.booking.value.location == 'office'
                                      ? "You will travel to provider's office"
                                      : controller.booking.value.location == 'home'
                                          ? "Your provider will come to your home"
                                          : "Not Mentioned",
                              style: Get.textTheme.bodyText1),
                        ):Container(),
                    duration > 0
                        ? BookingTilWidget(
                            title: Text("Booking Auto Cancellation",
                                style: Get.textTheme.subtitle2),
                            actions: [],
                            content: Column(
                              children: [
                                BookingRowWidget(
                                    description: "Booking expiring in",
                                    child: Align(
                                        alignment: Alignment.centerRight,
                                child: CountdownTimer(
                                  endTime: duration,
                                    ...
                                    hasDivider: false),
                                    Text("Your Booking shall be auto cancelled if provider does not accept this booking in the above-mentioned time", style: Get.textTheme.caption).paddingOnly(top: 10),
                              ],
                            ))
                        : Container(),
                    Obx(() {
                                    ...
                                    child: Align(
                                      ...
                                      child: Text(
                                            ...
                                            controller.booking.value.quantity
                                                .toString() +
                                            "x" +
                                            controller.booking.value.eService.getPrice.toString(),
                                        style: Get.textTheme.bodyText2,
                                      ),
                                    ),
                                    hasDivider: true),
