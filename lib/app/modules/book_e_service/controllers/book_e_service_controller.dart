...
class BookEServiceController extends GetxController {
  ...
  void createBooking() async {
    try {
      ...
      if (Get.isRegistered<EServiceController>()) {
        var trackId=Get.find<EServiceController>().trackid;
        if(trackId!=null)
        {
          print('Send Booked');
          await _bookingRepository.sendTrackBooked(trackId);
        }
      }
      Get.toNamed(Routes.CONFIRMATION, arguments: {'provider': booking.value.eProvider});
      EasyLoading.dismiss();
    } catch (e) {
      print(e);
