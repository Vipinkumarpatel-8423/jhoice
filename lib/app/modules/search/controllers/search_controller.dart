...
class SearchController extends GetxController {
  ...
  final categoriesWithSub = <Category>[].obs;
      ...
      if (selectedCategories.isEmpty) {
        ...
        categories.forEach((element) {
          element.subCategories.forEach((subcat) {
            cateList.add(subcat.id);
          });
        });
        eServices
            .assignAll(await _eServiceRepository.search(keywords, cateList));
      } else {
  ...
  Future getCategories() async {
    try {
      ...
      categoriesWithSub.assignAll(categories.toList());
      categories.forEach((element) {
        element.subCategories.forEach((subcat) {
          categoriesWithSub.add(subcat);
        });
      });
    } catch (e) {
      Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
