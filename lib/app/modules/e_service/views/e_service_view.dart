...
class EServiceView extends GetView<EServiceController> {
                        ...
                        EServiceTilWidget(
                          ...
                          content: Text(
                              _eService.mode == 'both'
                                  ...
                                  : _eService.mode == 'online'
                                      ? "This service can be done only online"
                                      : _eService.mode == 'offline'
                                          ? "This service can be done only offline"
                                          : "Not Mentioned",
                              style: Get.textTheme.bodyText1),
                        ),
                        EServiceTilWidget(
                          title: Text("Location of Service",
                              style: Get.textTheme.subtitle2),
                          content: Text(
                              _eService.location == 'both'
                                  ? "Provider can come to your house and you can also visit provider's office"
                                  : _eService.location == 'office'
                                      ? "You will have to travel to provider's office"
                                      : _eService.location == 'home'
                                          ? "Your provider will come to your home"
                                          : "Not Mentioned",
                              style: Get.textTheme.bodyText1),
                        ),
                        _eService.terms.isNotEmpty
                            ? EServiceTilWidget(
                                title: Text("Terms of Service",
                                    style: Get.textTheme.subtitle2),
                                content:
                                    buildTermsOfService(_eService, context),
                              )
                            : Container(),
                        _eService.special_points.isNotEmpty
                            ? EServiceTilWidget(
                                title: Text("Special Points",
                                    style: Get.textTheme.subtitle2),
                                content: Ui.applyHtml(_eService.special_points,
                                    style: Get.textTheme.bodyText1),
                              )
                            : Container(),
                        EServiceTilWidget(
  ...
  Widget buildTermsOfService(EService _eService, BuildContext context) {
    return MediaQuery.removePadding(
      ...
      child: ListView.builder(
          ...
          shrinkWrap: true,
          itemCount: _eService.terms.length,
          itemBuilder: (BuildContext context, int index) {
            return Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(_eService.terms[index],
                      style: Get.textTheme.bodyText1),
                ),
              ],
            );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(_eService.getDurationWithUnit, style: Get.textTheme.headline6),
        ],
      ),
    );
  }
            ...
            child: BlockButtonWidget(
                ...
                onPressed: () {
                  if (Get.isRegistered<AuthService>()) {
                    if (!Get.find<AuthService>().user.value.auth) {
                      // Get.toNamed(Routes.LOGIN);

                      // Get.showSnackbar(
                      //     Ui.ErrorSnackBar(message: "Please Login Now".tr));
                    } else if (Get.find<AuthService>().user.value.phoneNumber ==
                            null ||
                        Get.find<AuthService>().user.value.phoneNumber == '') {
                      Get.showSnackbar(Ui.ErrorSnackBar(
                          message:
                              "Please update your phone number in the app profile"
                                  .tr));
                      return;
                    }
                  }
                  controller.sendTrackClick('book');
